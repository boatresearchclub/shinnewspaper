...
def write_numeric_sheet(wb, all_race_data, course_master=None, venue_course_master=None):
    ...
    KOSATSU_ROWS = [
        # ① 展開ストーリー（逃げ判定・主役・崩れ・買い目）
        ("🎯展開ストーリー",  "①逃げ判定 ②逃時2・3着 ③主役 ④主役時2・3着 ⑤崩れ → 買い目"),
        # ② 展開パターン（A/B/C/D）と方針
        ("🎯展開パターン",  "A鉄板/B主役/C拮抗/D荒れ ＋ 方針"),
        # ③ 堅実度スコア（quality_score ベース）
        ("🎯堅実度",  "S(>=75)/A(>=55)/B(>=45)/C(>=35)/D(<35)"),
    ]
        ...
        for i, rd in enumerate(all_race_data):
            ...
            # ⑦展開quality
                ...
                ROI_GUIDE = {
                    ...
                    "両建て":     "ROI目安: 両建て=約32%(中央値¥1,400)  ※軸絞ること",
                }
