...
def calc_course_master(ippan: pd.DataFrame, raw_all: pd.DataFrame = None) -> pd.DataFrame:
    ...
    #
    # 緩和ロジック:
    #   全出走数 >= 200 かつ コース実績 >= 5 → グレードメイン選手と判定
    #     閾値を 15 → 5 に緩和（コース実績が本当に少ない場合は緩めない）
    #   全出走数 >= 100 かつ コース実績 >= 3 → 混合タイプ選手と判定
    #     閾値を 15 → 8 に緩和
    #   上記以外 → 従来どおり 15（新人・一般戦メインは影響なし）
    #
    # 安全設計:
    #   ・コース実績数が最低ラインを下回る場合は必ず通常閾値を適用
    #     → 「グレードメインだがこのコースは本当に実績なし」を誤爆しない
    #   ・新人（全出走数 < 100）は条件に引っかからず従来どおり
    #   ・一般戦ベテラン（全出走数 >= 100 かつ一般戦が主）も
    #     一般戦出走数 >= 15 あれば元々★が立たないので影響なし
    # ──────────────────────────────────────────────────────────────────────────

    # 全戦種合計出走数を選手別に集計して base にマージ（★判定のみに使用）
    _total_runs = df.groupby("選手名").size().reset_index(name="_全出走数_total")
    ...
    # グレードメイン判定（全出走数 >= 200 かつ コース実績 >= 5）
    _is_grade_main  = (_total >= 200) & (_course_runs >= 5)
    # 混合タイプ判定（全出走数 >= 100 かつ コース実績 >= 3）
    _is_mixed       = (_total >= 100) & (_course_runs >= 3)
    ...
    if raw_all is not None and len(raw_all) > 0:
        ...
        if len(g1sg_df) > 0:
            ...
            # 補完対象マスク: 出走数 < 20（★ST または ★1着率）かつG1/SGデータあり
            mask = (base["出走数"] < 15) & base["g1sg_出走数"].notna() & (base["g1sg_出走数"] > 0)
            n_補完 = mask.sum()
...
def calc_senshu_master(raw: pd.DataFrame, ippan: pd.DataFrame) -> pd.DataFrame:
    ...
    #
    # 分類ロジック:
    #   グレード出走割合 = (総出走数 - 一般戦出走数) / 総出走数
    #   >= 0.50 → "グレードメイン"  （SG/G1常連。一般戦★は実績不足であって弱くない）
    #   >= 0.20 → "混合"            （グレード・一般を行き来するタイプ）
    #   それ以外 → "一般メイン"     （新人・一般戦専門。★は純粋にデータ不足を意味する）
    #
    # 安全設計:
    #   ・総出走数が0の場合は "不明" として分類失敗を明示（NaNにしない）
    #   ・一般戦出走数が総出走数を上回るケース（データ不整合）も "一般メイン" に丸める
    #   ・lr_calc.py はこの列の存在チェックを行い、なければ従来動作に自動フォールバック
    #     → update_master.py を古いバージョンに戻しても lr_calc.py は壊れない
    # ──────────────────────────────────────────────────────────────────────────
    def _classify_player_type(row):
        total = row.get("総出走数", 0) or 0
        ippan_runs = row.get("一般戦出走数", 0) or 0
        if total == 0:
            return "不明"
        grade_ratio = max(0.0, (total - ippan_runs) / total)
        if grade_ratio >= 0.50:
            return "グレードメイン"
...
def calc_winner_course_analysis(ippan: pd.DataFrame) -> pd.DataFrame:
    ...
    base.drop(columns=["_2着合計", "_3連合計"], inplace=True)
...
def write_winner_course_analysis(wb, df: pd.DataFrame):
    ...
    COLS    = ["選手名", "進入コース", "勝者コース", "レース数", "平均着順", "2着率", "3着以内率", "信頼度"]
    HDRS    = ["選手名", "自コース",   "勝者コース", "レース数", "平均着順", "2着率", "3着\n以内率", "信頼度"]
    WIDTHS  = [14,       8,            8,            8,          8,          8,       8,             8]
    PCT_CI  = {6, 7}   # 2着率・3着以内率（1-indexed）
...
def main():
    ...
    if _is_ippan and len(raw) > 0 and len(ippan) > 0:
        ...
        try:
            ...
            _chk_df = pd.concat([_chk_total, _chk_ippan], axis=1).fillna(0).astype(int)
            _chk_df["グレード出走数"] = (_chk_df["全出走数"] - _chk_df["一般戦出走数"]).clip(lower=0)
            _chk_df["グレード比率"]   = (_chk_df["グレード出走数"] / _chk_df["全出走数"].replace(0, np.nan)).round(3)
            ...
            print(f"  グレードメイン(比率≥50%): {_n_gm:,}人  "
                  f"({_n_gm/_n_all*100:.1f}%)")
            print(f"  混合          (比率20-50%): {_n_mix:,}人  "
                  f"({_n_mix/_n_all*100:.1f}%)")
            print(f"  一般メイン    (比率< 20%): {_n_ip:,}人  "
                  f"({_n_ip/_n_all*100:.1f}%)")
            ...
            if _n_gm > 0:
                ...
                print(f"  中央値 : {int(_q[0.50]):,} 走  ← 閾値200の基準点")
                print(f"  75%ile : {int(_q[0.75]):,} 走")
                print(f"  90%ile : {int(_q[0.90]):,} 走")
                print(f"  最大値 : {int(_gm_total.max()):,} 走")

                # 現在の閾値200走で何人拾えているか
                _covered_200 = (_gm_total >= 200).sum()
                _covered_100 = (_gm_total >= 100).sum()
                _covered_pct_200 = _covered_200 / _n_gm * 100
                _covered_pct_100 = _covered_100 / _n_gm * 100

                print(f"\n【現在の閾値でグレードメイン選手を何人拾えているか】")
                print(f"  全出走数 ≥ 200（グレードメイン閾値）: "
                      f"{_covered_200:,}人 / {_n_gm:,}人 = {_covered_pct_200:.1f}%")
                print(f"  全出走数 ≥ 100（混合タイプ閾値）    : "
                      f"{_covered_100:,}人 / {_n_gm:,}人 = {_covered_pct_100:.1f}%")

                # 閾値200に届かないグレードメイン選手の一覧（上位20人）
                _missed = _chk_df.loc[_grade_main_mask & (_chk_df["全出走数"] < 200)].copy()
                if len(_missed) > 0:
                    _missed = _missed.sort_values("グレード比率", ascending=False)
                    print(f"\n  ⚠ 閾値200に届かないグレードメイン選手: {len(_missed)}人")
                    print(f"     （この選手たちは改修の恩恵を受けていません）")
                    print(f"     上位20人:")
                    ...
                    # 推奨閾値の提案
                    _recommend_200 = int(_gm_total.quantile(0.25))
                    print(f"\n  💡 推奨: 閾値200 → {_recommend_200} に下げると"
                          f"グレードメインの75%をカバーできます")
                else:
                    print(f"\n  ✅ 閾値200で全グレードメイン選手をカバーしています")
            ...
            if _n_mix > 0:
                ...
                print(f"  中央値: {int(_q_mix[0.50]):,} 走  ← 閾値100の基準点")
                print(f"  75%ile: {int(_q_mix[0.75]):,} 走")

                _missed_mix = (_mix_total < 100).sum()
                if _missed_mix > 0:
                    print(f"  ⚠ 閾値100に届かない混合選手: {_missed_mix}人 "
                          f"({_missed_mix/_n_mix*100:.1f}%)")
                    _recommend_100 = int(_mix_total.quantile(0.25))
                    print(f"  💡 推奨: 閾値100 → {_recommend_100} に下げると"
                          f"混合タイプの75%をカバーできます")
                else:
                    print(f"  ✅ 閾値100で全混合タイプ選手をカバーしています")

            # ── 誤爆チェック：新人・一般メインが緩和対象になっていないか ────
            print(f"\n【誤爆チェック：新人・一般メイン選手への影響】")
            # 全出走数200以上かつ一般戦メイン（グレード比率<20%）= 一般ベテラン
            _false_gm = ((_chk_df["全出走数"] >= 200) & _ippan_main_mask).sum()
            _false_mix = ((_chk_df["全出走数"] >= 100) & _ippan_main_mask).sum()
            print(f"  全出走200以上の一般メイン選手（閾値5に緩和）: {_false_gm}人")
            print(f"  全出走100以上の一般メイン選手（閾値8に緩和）: {_false_mix}人")
            if _false_gm > 0:
            ...
            print(f"\n【閾値設定の総合評価】")
            if _n_gm > 0:
                _cov = _covered_200 / _n_gm * 100
                if _cov >= 75:
                    print(f"  ✅ 現在の閾値200は概ね妥当です（カバー率{_cov:.1f}%）")
                elif _cov >= 50:
                    print(f"  ⚠ 閾値200のカバー率が{_cov:.1f}%です。"
                          f"下げることを検討してください")
                else:
                    print(f"  ❌ 閾値200のカバー率が{_cov:.1f}%と低すぎます。"
                          f"閾値の見直しを強く推奨します")
