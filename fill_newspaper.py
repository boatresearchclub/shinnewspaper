...
# ============================================================
# [lock] 日付ロックガード
#    load_race.py が書き出した .race_date_lock を確認し、
#    日付が一致しない場合はこのスクリプトを終了させる。
#    load_race.py から subprocess で呼ばれる場合も含め、
#    ロックファイルが存在しなければ実行を拒否する。
# ============================================================
def _check_date_lock(args_date: str | None) -> None:
    """
    .race_date_lock の内容と引数の日付を照合する。
    不一致・ファイル未存在の場合は sys.exit(1) で終了。
    """
    ...
    # ロックファイルが空（--date 未指定のまま race_date も未確定）は通す
    if not _locked_date:
        return

    # --date 引数が渡されている場合は一致チェック
...
try:
    ...
    print("[NG] xlwings がインストールされていません。")
...
def load_st_rank_master(wb):
    ...
    if sheet_name not in wb.sheetnames:
        print(f"[!]  シートが見つかりません: {sheet_name} → ST順位なしで処理")
        return {}
...
def load_fly_label(wb, players):
    ...
    【修正(2)】FLY経過日数を load_race.py と同じロジックで判定。
...
def ensure_fly_input_rows(wb, venue):
    ...
    if insert_after_row is None:
        print(f"  [!]  FLY入力行の挿入位置が見つかりませんでした")
        return
    ...
    print(f"  [OK] {sheet_name} に FLY入力行を追加しました（Row{insert_row}〜{insert_row+5}）")
...
def read_numeric_sheet(wb, venue):
                ...
                "players": [
                    {
                        ...
                        "sanren_idx": 75.0,  # 3連対指数（0〜100、会場×コース補正済み相対スコア）
    ...
    sheet_name = f"{venue}_数値"
    if sheet_name not in wb.sheetnames:
        print(f"[NG] シートが見つかりません: {sheet_name}")
    ...
    if not race_col_map:
        print(f"[NG] {sheet_name} にレース列が見つかりません")
        return None
    ...
    for r_idx, row_vals in enumerate(all_rows):
        ...
        if rank_row is None and cat is not None:
            c1 = str(cat)
            c2 = str(item or "")
            if (c1 == "判定" and c2.startswith("ランク")) or c1 == "(1)判定スコア" or "堅実度" in c1:
                rank_row = r
    ...
    for race_no, rc in race_col_map.items():
        ...
        # ランク・スコア：rank_row（(1)判定スコア行）を優先、次にstep2_row
        # (1)判定スコア行のD列: 「【ランクS】75点\n戦略: ...\n信頼度: ...%」形式
        rank  = "-"
        score = None
        _score_rows = []
        if rank_row:
        ...
        for waku in range(1, 7):
            players.append({
                ...
                "sanren_idx":   get_val("数値指標_3連対指数",                                   waku, col),
                "course_sanren":pct_to_float(get_val("数値指標_コース別3連対率(%)",              waku, col)),
                "circle_pct":   pct_to_float(get_val("数値指標_2着優位度(%)[相対・イン逃げ時]", waku, col)),
                "idx3":       get_val("数値指標_3着指数", waku, col),
                "jizen": {
                    "in_nige":  get_val("事前評価_(1)逃げ",   waku, col),
                    "aisho":    get_val("事前評価_(2)相性",   waku, col),
                    "kiryoku":  get_val("事前評価_(3)機力",   waku, col),
                    "tenkai":   get_val("事前評価_(4)展開",   waku, col),
                    "jizaisei": get_val("事前評価_(5)S安定",  waku, col),
                },
                "kimari": {
                ...
                "avg_st":    get_val("選手情報_コース別ST順位", waku, col),
            })
        ...
        strategy        = None
        trust_pct       = None
        kosatsu_raw     = None   # (8)考察の結論（生テキスト）
        tenkai_raw      = None   # (4)展開予測（生テキスト）
        ...
        for r in range(1, max_r + 1):
            ...
            if v1 == "(1)判定スコア" and strategy is None:
            ...
            # (8)考察の結論・(4)展開予測を追加取得
            # ※数値シートでは「(8)考察の結論」は存在せず、A列「(4)展開予測」/B列「展開◎○艇の役割」が相当する。
            #   A列ラベル（v1）でも B列ラベル（v2）でも両方チェックしてフォールバック取得する。
            if ("(8)考察の結論" in v2 or "(8)考察の結論" in v1) and kosatsu_raw is None:
                kosatsu_raw = str(cell_cache.get((r, col), "") or "")
            if ("(4)展開予測" in v2 or "(4)展開予測" in v1) and tenkai_raw is None:
                tenkai_raw = str(cell_cache.get((r, col), "") or "")
            # 「展開◎○艇の役割」（(4)展開予測のB列ラベル）で取得できていない場合のフォールバック
...
def fill_one_race(ws, race_no, venue, race_date, race_data, st_ranks_by_waku=None,
    ...
    for i, p in enumerate(players[:6]):
        ...
        # 同コース20走未満の選手は名前の前に ※ を追加
        name_raw = p["name"]
        if waku in _missing:
            name_display = f"[!]　{name_raw}"
        elif p.get("scarce_mark"):
            name_display = f"※{name_raw}"
        else:
        ...
        # O列（列15）: 3連対指数（整数 0〜100）
        sanren_val = p.get("sanren_idx")
        wv(row, 15, round(float(sanren_val)) if sanren_val is not None else None)
    ...
    # Row 14〜19: 事前評価（B〜F列）+ 3着指数（L列）+ 一般3連対率（N列）
    # テンプレート列配置: B=逃げ, C=相性, D=機力, E=自在性, F=展開, L=3着指数, N=一般3連対率
    JIZEN_COL_MAP = [
        ("in_nige",  2),   # B列: (1)逃げ
        ("aisho",    3),   # C列: (2)相性
        ("kiryoku",  4),   # D列: (3)機力
    ...
    for i, p in enumerate(players[:6]):
        ...
        # N列（列14）: 一般3連対率＝コース別3連対率（例: 85.7% → '86%'）
        wv(row, 14, fmt_pct(p.get("course_sanren")))
    ...
    # ── 20走未満の選手が1人でもいる場合は考察・買い目を出力しない ──────────
    # 同コース出走数が少ない選手のデータは信頼性が低く、
    # 考察・買い目が誤った方向を示すリスクがあるため非表示にする。
    _has_scarce = any(p.get("scarce_mark") for p in players)
    if _has_scarce:
        _scarce_names = "・".join(
            p["name"] for p in players if p.get("scarce_mark")
        )
        wv(22, 9, f"※出走数不足の選手がいるため考察・買い目は非表示です。\n（対象: {_scarce_names}）")
        wv(25, 9,  None)
        wv(25, 12, None)
        wv(29, 12, None)
        return  # 以降の考察・買い目書き込みをスキップ
    ...
    # ── (1)考察文 → I22（数値シート144行 Step1 のD列テキスト）────────────
    ...
    # ── (2)参考買い目 → 数値シート145行「主役展開」のD列テキスト ──────────
    # 「参加見送り」「見送り推奨」「※」行を除去。
    ...
    # ── (3)理想合成オッズ → L29 明示的にクリア（テンプレートの残存値を消す）
...
def main():
    ...
    parser.add_argument("--date",  type=str, default=None, help="対象日付 (例: 2026-03-30)。指定時はシート日付が一致する会場のみ処理")
    ...
    if not all_candidates:
        print("[NG] 数値シートが見つかりません。先に load_race.py を実行してください。")
        return

    # ── 各会場シートの日付を収集 ─────────────────────────────────────
    venue_date_map = {}  # {venue: "YYYY-MM-DD" or ""}
    for v in all_candidates:
        ws_tmp = wb_num[f"{v}_数値"]
        hdr    = str(ws_tmp.cell(1, 1).value or "")
        m      = re.search(r"(\d{4}-\d{2}-\d{2})", hdr)
        venue_date_map[v] = m.group(1) if m else ""

    # 利用可能な日付一覧（降順）
    available_dates = sorted(
        {d for d in venue_date_map.values() if d},
        reverse=True
    )

    if args.all:
        venues_to_process = all_candidates
        print(f"  [完了] 全会場一括処理モード: {', '.join(venues_to_process)}")
    elif args.venue:
        venues_to_process = [args.venue]
        print(f"  [完了] 会場: {args.venue}")
    else:
        # ── (1) 日付選択 ────────────────────────────────────────────────
        selected_date = None
        if not args.date:
            if len(available_dates) == 1:
                ...
                print(f"  [日付] 日付: {selected_date}（自動選択）")
            elif len(available_dates) > 1:
                print()
                print("  [日付] 対象日付を選択してください:")
                for i, d in enumerate(available_dates, start=1):
                    venues_on_date = [v for v, vd in venue_date_map.items() if vd == d]
                    label = "（最新）" if i == 1 else ""
                    print(f"    {i}. {d}{label}  [{', '.join(venues_on_date)}]")
                print()
                try:
                    dsel = input(f"  番号を入力 (1〜{len(available_dates)}, Enter=最新): ").strip()
                except (EOFError, KeyboardInterrupt):
                    print("\n[NG] キャンセルしました")
                    return
                if dsel == "" or dsel == "1":
                    selected_date = available_dates[0]
                elif dsel.isdigit() and 1 <= int(dsel) <= len(available_dates):
                    selected_date = available_dates[int(dsel) - 1]
                else:
                    print(f"[NG] 無効な番号です: {dsel}")
                    return
                print(f"  [日付] 選択日付: {selected_date}")
        else:
            selected_date = args.date

        # 選択日付で絞り込んだ会場リスト
        if selected_date:
            date_filtered = [v for v in all_candidates if venue_date_map.get(v) == selected_date]
            if not date_filtered:
                print(f"[NG] 日付 {selected_date} のデータを持つ会場が見つかりません。")
                return
            all_candidates_for_select = date_filtered
        else:
            all_candidates_for_select = all_candidates

        # ── (2) 会場選択 ────────────────────────────────────────────────
        print()
        print("  [フォルダ] 会場を選択してください:")
        print(f"    0. 全会場（一括処理）")
        for i, v in enumerate(all_candidates_for_select, start=1):
            print(f"    {i}. {v}")
        print()
        try:
            sel = input(f"  番号を入力 (0=全会場, 1〜{len(all_candidates_for_select)}): ").strip()
        except (EOFError, KeyboardInterrupt):
            print("\n[NG] キャンセルしました")
            return
        if sel == "0":
            venues_to_process = all_candidates_for_select
            print(f"  [完了] 全会場一括処理モード: {', '.join(venues_to_process)}")
        elif not sel.isdigit() or not (1 <= int(sel) <= len(all_candidates_for_select)):
            print(f"[NG] 無効な番号です: {sel}")
            return
        else:
            venues_to_process = [all_candidates_for_select[int(sel) - 1]]
            print(f"  [完了] 会場: {venues_to_process[0]}")

        # --date 引数の代わりに selected_date を使用（以降のフィルタに反映）
        if selected_date and not args.date:
    ...
    for attempt in range(3):
        try:
            ...
            if attempt < 2:
                print(f"  [待機] 出力ファイルがロック中... 3秒後にリトライ ({attempt+1}/3)")
                time.sleep(3)
            else:
                print(f"[NG] 出力ファイルを書き込めません。手動でExcelを閉じてから再実行してください: {OUTPUT_PATH.name}")
                return
    ...
    for venue in venues_to_process:
        ...
        if venue_stats:
            ...
            print(f"  [OK] 場平均1着率: {win1*100:.1f}% / イン逃げ率: {in_r*100:.1f}%" if win1 and in_r else "  [!]  会場統計データなし")
        else:
            print(f"  [!]  会場統計シートが見つからないか、{venue}のデータがありません")
        ...
        if not fly_already_exists:
            try:
                ...
                print(f"  [!]  FLY入力行の追加に失敗しました（続行します）: {e}")

        races = read_numeric_sheet(wb_num, venue)
        if not races:
            print(f"  [!]  {venue}: 数値シートが読み取れないためスキップ")
            continue
        ...
        print(f"  [OK] F/ST影響: FLYあり {len(fly_msgs)}艇検出")

        # 日付・レース番号（venue_date_mapキャッシュを優先利用）
        race_date = venue_date_map.get(venue, "")
        if not race_date:
        ...
        # ── 日付フィルタ: --date 指定時はシート日付が一致する会場のみ処理 ──
        if args.date and race_date and race_date != args.date:
            print(f"  [スキップ]  [{venue}] シート日付({race_date}) ≠ 指定日付({args.date}) → スキップ")
            continue
        if args.date and not race_date:
            print(f"  [!]  [{venue}] シートから日付を取得できませんでした → スキップ")
            continue
        ...
        if args.race:
            ...
            if not race_nos:
                print(f"  [!]  {venue}: {args.race}Rのデータが見つかりません（スキップ）")
                continue
        else:
            race_nos = all_race_nos
        print(f"  [表] [{venue}] 対象レース: {race_nos}")
    ...
    if not venue_data:
        print("[NG] 処理できる会場がありませんでした。")
        return

    print()
    print(f"  [フォルダ] Excelを起動中...")
    ...
    try:
        ...
        for venue, vd in venue_data.items():
            ...
            for race_no in race_nos:
                ...
                if missing:
                    print(f"  [!]  {race_no}R 艇番 {missing} のST順位が取得できませんでした（平均値で代替）")
                ...
                print(f"  [OK] {venue} {race_no}R 書き込み完了")
        ...
        print(f"  [保存] 保存中...")
        wb_out.save()
        print(f"  [OK] 完了！ → {OUTPUT_PATH.name}")
    ...
    try:
        ...
        print(f"  [画像]  スリット図をST順位で更新完了（{len(png_bytes_by_sheet_idx)}レース分）")
    except Exception as e:
        print(f"  [!]  スリット図の更新に失敗しました: {e}")
