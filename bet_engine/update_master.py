...
_GRADE_SUFFIX = {"一般": "", "G1": "_g1", "G2": "_g2", "G3": "_g2", "SG": "_g1"}
...
# グレードマスタを読み込んでグレード列を付与
    ...
    # グレードで絞り込み
    # target_grade="一般" → SG/G1〜G3を除外（従来通り）
    # target_grade="G1"   → G1/G2/G3のみ抽出
    # target_grade="SG"   → SGのみ抽出
    ...
    if _tg == "一般":
        ...
        ippan = raw[raw["グレード"].isin({"G1", "SG"})].copy()
        _label = "G1/SG"
    elif _tg == "G2":
        ippan = raw[raw["グレード"].isin({"G2", "G3"})].copy()
        _label = "G2/G3"
    else:
        EXCLUDE_GRADES = {"SG", "G1", "G2", "G3"} | ALWAYS_EXCLUDE
        ippan = raw[~raw["グレード"].isin(EXCLUDE_GRADES)].copy()
...
def calc_tenkai_survival_master(ippan: pd.DataFrame):
    ...
    _tv_out  = TENKAI_VENUE_CSV.replace(".csv", f"{_sfx}.csv")    if _sfx else TENKAI_VENUE_CSV
    _tn_out  = TENKAI_NATIONAL_CSV.replace(".csv", f"{_sfx}.csv") if _sfx else TENKAI_NATIONAL_CSV
    df_venue.to_csv(_tv_out, index=False, encoding="utf-8-sig")
    df_national.to_csv(_tn_out, index=False, encoding="utf-8-sig")
    print(f"  💾 CSV保存: {os.path.basename(_tv_out)}, {os.path.basename(_tn_out)}")
...
def write_venue_course_master(wb, df: pd.DataFrame):
    ...
    try:
        ...
        _vc_out = VENUE_COURSE_CSV.replace(".csv", f"{_sfx}.csv") if _sfx else VENUE_COURSE_CSV
        df[save_cols].to_csv(_vc_out, index=False, encoding="utf-8-sig")
        print(f"  会場別コースマスタCSV: {_vc_out} に書き出し完了")
    except Exception as e:
...
def main():
    ...
    _sheet_suffix = {"一般": "", "G1": "_G1", "G2": "_G2"}.get(_effective_grade, "")
    ...
    if _is_ippan:
        ...
        print(f"\n[気象統計・風種統計] {target_grade}モードのためスキップ（一般戦データを継続使用）")

    # シート順序を整える
    desired_order = [
        "📊コース別マスタ", "📊コース別マスタ_G1", "📊コース別マスタ_G2",
        "選手指数マスタ", "会場別コースマスタ",
        "会場統計", "イン逃げ分析", "イン逃げ分析_G1", "イン逃げ分析_G2",
        "展開別残存_会場別", "展開別残存_全国",
