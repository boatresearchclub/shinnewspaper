...
def main():
    ...
    if _collect_only:
        ...
        if _n_workers == 1 or _n_tasks == 1:
            ...
            print(f"  🚀 {_n_tasks}タスク（会場×日付）を {_n_workers}並列で処理します...")
    ...
    for venue in venues_to_process:
        ...
        for race_no in race_nos:
            ...
            # ════════════════════════════════════════════════════════════
            # ❷ 指数計算＋事前評価 コンソール出力
            #    calc_race_indices 内で❶号艇逃げ/主役/穴候補は出力済み。
            ...
            try:
                ...
                if _buy_list:
                    ...
                    print()
                    print(f"  ┌─ ❹ {race_no}R 買い目 {_skip_label}({len(_buy_list)}点){_syn_str} ─────────────")
                    # 本線
                    if _honsen:
                    ...
                    print()
                    print(f"  ── ❹ {race_no}R 買い目なし（見送り）: "
                          f"{bet_suggestions.get('skip_reason', '')} ──")
            except Exception as _e4:
                print(f"  [!]  ❹出力エラー（スキップ）: {_e4}")
            # ────────────────────────────────────────────────────────────
