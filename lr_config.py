...
# 会場別コースマスタのParquetキャッシュ（update_master.py が生成）
VENUE_COURSE_CSV = BASE_DIR / "data" / "venue_course_master.parquet"
# 展開別残存マスタのParquetキャッシュ（update_master.py が生成）
TENKAI_VENUE_CSV    = BASE_DIR / "data" / "tenkai_survival_venue.parquet"
TENKAI_NATIONAL_CSV = BASE_DIR / "data" / "tenkai_survival_national.parquet"

# ── ②ST差×決まり手 閾値マスタ（update_master.py calc_st_kimete_threshold で生成）──
ST_KIMETE_CSV = BASE_DIR / "data" / "st_kimete_threshold.parquet"

# ── ③コース開放連鎖マスタ（update_master.py calc_kaiho_chain_master で生成）──
KAIHO_VENUE_CSV    = BASE_DIR / "data" / "kaiho_chain_venue.parquet"
KAIHO_NATIONAL_CSV = BASE_DIR / "data" / "kaiho_chain_national.parquet"

# ── グレード別Parquetキャッシュ（update_master.py --grade g1 / --grade sg で生成）──
# ファイルが存在しない場合は一般戦マスタにフォールバックする
VENUE_COURSE_CSV_G1 = BASE_DIR / "data" / "venue_course_master_g1.parquet"
VENUE_COURSE_CSV_SG = BASE_DIR / "data" / "venue_course_master_sg.parquet"
TENKAI_VENUE_CSV_G1    = BASE_DIR / "data" / "tenkai_survival_venue_g1.parquet"
TENKAI_NATIONAL_CSV_G1 = BASE_DIR / "data" / "tenkai_survival_national_g1.parquet"
TENKAI_VENUE_CSV_SG    = BASE_DIR / "data" / "tenkai_survival_venue_sg.parquet"
TENKAI_NATIONAL_CSV_SG = BASE_DIR / "data" / "tenkai_survival_national_sg.parquet"
...
# G2・G3 は独立マスタ
_GRADE_CSV_MAP["G2"] = {
    "venue_course":      BASE_DIR / "data" / "venue_course_master_g2.parquet",
    "tenkai_venue":      BASE_DIR / "data" / "tenkai_survival_venue_g2.parquet",
    "tenkai_national":   BASE_DIR / "data" / "tenkai_survival_national_g2.parquet",
...
# 【⑤追加】会場別コース距離補正値Parquetキャッシュ（update_master.py が生成）
VENUE_COURSE_ADJ_CSV = BASE_DIR / "data" / "venue_course_adj.parquet"
...
def _load_venue_course_adj() -> dict:
    if not VENUE_COURSE_ADJ_CSV.exists():
        return {}
    try:
        df = pd.read_parquet(str(VENUE_COURSE_ADJ_CSV))
