...
def load_masters(wb, race_grade: str = "一般"):
    ...
    if _vc_csv_path.exists():
        try:
            ...
            print(f"  [!]  会場別コースマスタCSV読み込み失敗、Excelにフォールバックします: {e}")
            venue_course_master = {}
    ...
    if ST_KIMETE_CSV.exists():
        try:
            _df_stk = pd.read_csv(str(ST_KIMETE_CSV), encoding="utf-8-sig", dtype=str)
    ...
    for _csv_path, _master, _key_cols in [
        ...
        if _csv_path.exists():
            try:
                _df_kh = pd.read_csv(str(_csv_path), encoding="utf-8-sig", dtype=str)
