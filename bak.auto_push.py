...
CHECK_INTERVAL = 10  # 秒（展示・オッズ検知を高速化）
...
def calc_prob_from_master(boats, venue, race_no=0, is_joshi=False):
    # 女子戦は course_master_joshi を優先使用。存在しない場合は一般戦マスタにフォールバック
...
def parse_csv(filepath):
    ...
    # ── 女子戦フラグを race_index_{YYYYMMDD}.json から取得（period照合で正確に判定）──
    is_joshi = False
    try:
    ...
    for rno, rd in races.items():
        rd["boats"] = calc_prob_from_master(rd["boats"], venue, race_no=rno, is_joshi=is_joshi)
...
def _write_and_push_odds_json(saved_paths: list) -> bool:
    ...
    code, _ = run(["git", "push", "origin", "main"])
    if code != 0:
        code, _ = run(["git", "push", "origin", "master"])
    if code == 0:
...
def _start_odds_loop_if_needed() -> None:
    ...
    """
    global _odds_worker_thread, _odds_worker_stop
...
def _clear_git_lock():
    ...
    if lock_file.exists():
        try:
            ...
            log(f"  [git] index.lock 削除失敗: {e}")

def run(cmd):
    # git commit / git add の前に index.lock を自動クリア
    if len(cmd) >= 2 and cmd[0] == "git" and cmd[1] in ("commit", "add"):
        _clear_git_lock()
...
def get_today_csvs():
    """今日の日付が含まれるCSVだけを返す"""
    today = today_str()
    return [p for p in glob.glob(str(CSV_DIR / "*.csv"))
            if today in Path(p).name]
...
def update_cache_version():
    """index.html の /* __CACHE_VER__ */ をタイムスタンプで書き換える"""
    try:
        text = INDEX_HTML.read_text(encoding="utf-8")
        ver  = datetime.now().strftime("%Y%m%d%H%M%S")
        import re as _re
        text = _re.sub(r'/\* __CACHE_VER__ \*/', ver, text)
...
def git_push(changed_files):
    ...
    if PARAMS_JS.exists():
        ...
        if params_changed or not PARAMS_JS_OBF.exists():
            ...
            run(["git", "add", str(obf_path_params)])
        else:
            run(["git", "add", str(PARAMS_JS_OBF)])
    ...
    if CSV_EXPORT_JS.exists():
        ...
        if csv_changed or not CSV_EXPORT_JS_OBF.exists():
            ...
            update_cache_version()
            run(["git", "add", str(obf_csv)])
        else:
            run(["git", "add", str(CSV_EXPORT_JS_OBF)])
    ...
    if SIM_JS.exists():
        ...
        if sim_changed or not SIM_JS_OBF.exists():
            ...
            update_cache_version()
            run(["git", "add", str(obf_sim)])
        else:
            run(["git", "add", str(SIM_JS_OBF)])
    ...
    if BACKTEST_JS.exists():
        ...
        if bt_changed or not BACKTEST_JS_OBF.exists():
            ...
            update_cache_version()
            run(["git", "add", str(obf_bt)])
        else:
            run(["git", "add", str(BACKTEST_JS_OBF)])
    ...
    if TOP_STATS_JS.exists():
        ...
        if ts_changed or not TOP_STATS_JS_OBF.exists():
            ...
            update_cache_version()
            run(["git", "add", str(obf_ts)])
        else:
            run(["git", "add", str(TOP_STATS_JS_OBF)])
    ...
    if TOP_PAGE_JS.exists():
        ...
        if tp_changed or not TOP_PAGE_JS_OBF.exists():
            ...
            update_cache_version()
            run(["git", "add", str(obf_tp)])
        else:
            run(["git", "add", str(TOP_PAGE_JS_OBF)])

    # calibration.js（難読化なし・そのままpush）
    if CALIBRATION_JS.exists():
        run(["git", "add", str(CALIBRATION_JS)])   # ← 追加ブロック
    ...
    if JS_FILE.exists():
        ...
        if js_changed or not JS_FILE_OBF.exists():
            # sample.js が変更された or 難読化済みファイルがない → 難読化 + キャッシュバスター更新
            obf_path = obfuscate_js(JS_FILE, JS_FILE_OBF)
            update_cache_version()   # JSが変わったときだけバージョンを更新 → 不要なリロードを防ぐ
            run(["git", "add", str(obf_path)])
        else:
            # 変更なし → そのまま add だけ（既に git 管理下にあれば変更なしとして扱われる）
            run(["git", "add", str(JS_FILE_OBF)])

    if DATA_JS.exists():
        run(["git", "add", str(DATA_JS)])
    if PLAYER_ID_MAP.exists():
        run(["git", "add", str(PLAYER_ID_MAP)])
...
def main():
    ...
    prev_csv   = get_mtimes(str(CSV_DIR / f"*{today_str()}*.csv"))
    ...
    if today_csvs:
        ...
        log("  ✓ 展示・コメント・オッズ push完了（買い目再計算はバックグラウンドで実行中）")

        # ② 重い買い目再計算はバックグラウンドで実行 → 完了後に追加push
        import threading as _threading
        def _reprocess_bg():
            log("  [BG] 買い目再計算 開始...")
            ...
            # フェーズ3: HTMLへの埋め込みを停止 → data/*.json + fetchに完全移行
            # inject_result_to_html()  # 停止: data/result_YYYYMMDD.json でfetch配信
            # inject_history_to_html() # 停止: data/history_YYYYMMDD.json でfetch配信
    ...
    try:
        while True:
            ...
            curr_csv   = get_mtimes(str(CSV_DIR / f"*{today_str()}*.csv"))
            ...
            if changed:
                ...
                if tenji_or_comment_changed:
                    ...
                    for tj in DATA_DIR.glob("tenji_*.json"):
                        run(["git", "add", str(tj)])
                    code2, out2 = run(["git", "status", "--porcelain"])
                    tracked2 = [l for l in out2.strip().splitlines() if not l.startswith("??")]
                    if tracked2:
                        run(["git", "commit", "-m", f"tenji update {datetime.now().strftime('%Y-%m-%d %H:%M')}"])
                        code2, _ = run(["git", "push", "origin", "main"])
                        if code2 != 0:
                            run(["git", "push", "origin", "master"])
                        log("  ✓ 展示情報 先行push完了")
                else:
